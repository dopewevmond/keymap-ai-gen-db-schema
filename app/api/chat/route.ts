...
export async function POST(req: Request) {
  try {
    ...
    let content = `You are a database design architect responsible for generating a database schema based on app ideas provided by the user.
            The id of the table should be the same as the name of the table. The source table and source column are the entity that can be related to more than one instance of the target table and target column. Even if you modify only one entity return the whole schema.
